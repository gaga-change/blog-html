...
exports.search = async (params) => {
    let sql = `SELECT SQL_CALC_FOUND_ROWS id,create_time,title,intro,comment_num,click_num,tags,display_name,modify_time,classify_name,private FROM article_post_public ${tools.where(params.where)} ${tools.order(params.order)} LIMIT ?, ?`
    let date = Date.now()
